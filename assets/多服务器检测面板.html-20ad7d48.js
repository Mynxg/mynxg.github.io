...
<span class="token comment"># systemctl stop stat_server</span>
<span class="token comment"># systemctl stop stat_client</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>修改配置：</p><p><img src="`+v+'" alt="" loading="lazy"> 去掉注释，也就是去掉#号</p><p>后面的地址替换成自己服务端反向代理的域名</p>',8),A={href:"https://w.lxip.top",target:"_blank",rel:"noopener noreferrer"},H={href:"https://w.lxip.top/report",target:"_blank",rel:"noopener noreferrer"},P=s("p",null,"如果不想反向代理，或者觉得麻烦，那就直接使用服务器公网ip地址+端口号",-1),E={href:"http://139.212.12.120:8080",target:"_blank",rel:"noopener noreferrer"},W=t('<p>服务端与客户端的关联关系：</p><figure><img src="'+u+'" alt="image-20230626012319564" tabindex="0" loading="lazy"><figcaption>image-20230626012319564</figcaption></figure><figure><img src="'+m+'" alt="image-20230626012950615" tabindex="0" loading="lazy"><figcaption>image-20230626012950615</figcaption></figure><blockquote><p>config.toml文件对应服务端</p></blockquote><p>完成配置如下：</p><figure><img src="'+b+'" alt="image-20230626013208676" tabindex="0" loading="lazy"><figcaption>image-20230626013208676</figcaption></figure><h2 id="_3-客户端搭建" tabindex="-1"><a class="header-anchor" href="#_3-客户端搭建" aria-hidden="true">#</a> 3 客户端搭建</h2>',7),I={href:"http://client.sh",target:"_blank",rel:"noopener noreferrer"},K=t(`<div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>#!/bin/bash
set -ex
...
<span class="token comment"># 状态查看</span>
systemctl status stat_client
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>都安装完成后如图所示： <img src="`+g+`" alt="" loading="lazy"></p><p>将客户端注册到服务端上：</p><p>打开 stat_client.service 文件：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>vim /etc/systemd/system/stat_client.service
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>修改如下图所示：</p><figure><img src="`+k+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>配置修改如下：</p><figure><img src="'+_+`" alt="image-20230626014148113" tabindex="0" loading="lazy"><figcaption>image-20230626014148113</figcaption></figure><h2 id="搭建nginx反向代理" tabindex="-1"><a class="header-anchor" href="#搭建nginx反向代理" aria-hidden="true">#</a> 搭建nginx反向代理</h2><blockquote><p>主要是用于将带有端口号的地址反向代理为一个域名（二级域名）</p><p>如果你使用ip地址+端口号的形式访问就不需要反向代理了，也就省下这一步骤。</p></blockquote><h2 id="卸载服务端和客户端" tabindex="-1"><a class="header-anchor" href="#卸载服务端和客户端" aria-hidden="true">#</a> 卸载服务端和客户端：</h2><ol><li>先停掉 之前开启的服务 查看服务状态：</li></ol><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>systemctl status stat_server
systemctl status stat_client
...
systemctl stop stat_client
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>再执行简单粗暴的方式删除文件夹的文件 在/opt/文件下删除ServerStatus文件夹（包括ServerStatus文件夹所有内容）</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>rm -rf ServerStatus
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><blockquote><p>注意：别使用 /* 来删除所文件，小心一不小心就删库了，对于第一次使用删除命令来删除文件夹下所有内容，是很容易犯这样的问题的。 原因：大致是习惯性使用可视化界面，把那种思维方式带到linux下，以为linux跟可视化界面删除方式删除方式相同，这就容易误操作，删库了。禁止这么操作，这是很严重的问题。</p></blockquote><p>删除完成后使用 ll命令，查看一下是否删除完成</p><p>然后还需要删除残余文件：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>rf -rf /etc/systemd/system/stat_server.service
rf -rf /etc/systemd/system/stat_client.service
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>不确定是否有这个两个文件，可以切换到 /etc/systemd/system/目录下检查是否有这个两文件，再做删除操作。</p><figure><img src="`+h+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><h2 id="总结" tabindex="-1"><a class="header-anchor" href="#总结" aria-hidden="true">#</a> 总结</h2><p>知道理解服务端与客户端之间如何关联起来。</p><p>恕我太愚钝，花了一天多的时间搭建这，尝试了多种方式终于知道怎么看对应的脚步和文档了，小有收获。终于弄明白了severstatus_rust不同服务器之间如何相关联，也对脚本有了更好的认识。</p><h2 id="附件" tabindex="-1"><a class="header-anchor" href="#附件" aria-hidden="true">#</a> 附件</h2>',32),j={href:"https://blog.laoda.de/archives/ward-serverstatus-instal",target:"_blank",rel:"noopener noreferrer"},L={href:"https://iocky.com/archives/13",target:"_blank",rel:"noopener noreferrer"},N={href:"https://doc.ssr.rs/deploy/",target:"_blank",rel:"noopener noreferrer"};function V(B,F){const n=c("ExternalLinkIcon");return l(),r("div",null,[x,s("blockquote",null,[s("p",null,[e("项目地址："),s("a",S,[e("zdz/ServerStatus-Rust: ✨ Rust 版 ServerStatus 探针、威力加强版 (github.com)"),a(n)])]),s("p",null,[e("官方教程："),s("a",y,[e("安装部署 - Rust 版 ServerStatus 云探针 (ssr.rs)"),a(n)])]),s("p",null,[e("在线地址："),s("a",q,[e("ServerStatus (ssr.rs)"),a(n)])]),z]),R,s("p",null,[e("下载："),s("a",w,[e("Release v1.7.2 · zdz/ServerStatus-Rust (github.com)"),a(n)])]),O,s("p",null,[s("a",$,[e("server.sh"),a(n)]),e(" 配置：")]),C,s("p",null,[e("例如你的域名是："),s("a",A,[e("https://w.lxip.top"),a(n)])]),s("p",null,[e("则替换后就是"),s("a",H,[e("https://w.lxip.top/report"),a(n)])]),s("blockquote",null,[P,s("p",null,[e("例如地址："),s("a",E,[e("http://139.212.12.120:8080"),a(n)]),e(" （服务端上的），直接替换也行")])]),W,s("p",null,[s("a",I,[e("client.sh"),a(n)]),e(" 配置")]),K,s("p",null,[e("参考1："),s("a",j,[e("搭建两个专属自己的服务器监控——Ward && ServerStatus"),a(n)]),e(" 参考2："),s("a",L,[e("ServerStatus-Rust 服务器探针搭建使用"),a(n)])]),s("p",null,[e("参考3："),s("a",N,[e("安装部署 - Rust 版 ServerStatus 云探针 (ssr.rs)"),a(n)])])])}const Q=i(f,[["render",V],["__file","多服务器检测面板.html.vue"]]);export{Q as default};
